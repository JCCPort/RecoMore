...
namespace client {
	...
	bool parse_numbers(Iterator first, Iterator last, std::vector<float>& v) {
		...
		const bool r = phrase_parse(first, last,
		                            ...
		                            //  Begin grammar
		                            (float_[push_back(phoenix::ref(v), _1)] >> *(' ' >> float_[push_back(phoenix::ref(v), _1)])),
		                            //  End grammar
...
DigitiserRun ReadWCDataFileDat(const std::string& fileName, const bool positivePulse) {
	...
	std::regex timeRegex("(=== UnixTime = (\\d*\\.\\d*)" " date = (\\d*\\.\\d*\\.\\d*)" " time = (\\d*h\\.\\d*m\\.\\d*s\\.\\d*ms) == " "TDC = (\\d*) == " "TDC corrected time = (\\d*h\\d*m\\d*s,\\d*\\.\\d*\\.\\d*ns) == "
	                     "Nb of channels = (\\d*) ===\r\n)");
...
DigitiserRun ReadWCDataFileBinary(const std::string& fileName, const bool positivePulse) {
	...
	constexpr float        ADC2mV             = (2500.f / 4096.f);
	...
	while (input_file.read(reinterpret_cast<char*>(&event_), sizeof(event_))) {
		...
		if (unsigned long long int previousEventTDC = 0; event_.TDCSAMIndex < previousEventTDC) {
			TDCOverflowCounter++;
		}
		...
		event.correctedTime = ssh.str() + "h" + ssm.str() + "m" + sss.str() + "s" + "," + subSec.substr(2, 3) + "." + subSec.substr(5, 3) + "." + subSec.substr(8, 3) + "ns";
