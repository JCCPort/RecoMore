...
def makeWaveformArray(waveform: ChannelWF, RMEvent: RecoMoreEvent):
    pdf = pd.read_csv("../pdf/ch{}.txt".format(waveform.channelNumber), sep="\t", names=['time', 'amp'])
    amps = pdf['amp'].values
    ...
    sumYs = np.zeros(len(waveform.rawData))
