...
def makeWaveformArray(waveform, RMEvent):
    pdf = pd.read_csv("../PEFinder/templates/ch{}.txt".format(waveform.channel), sep="\t", names=['time', 'amp'])
    # templates = pd.read_csv("/home/joshuap/PycharmProjects/SiPMSimulations/OutputDataForRecoMore/templates/ch{}.txt".format(waveform.channel), sep="\t", names=['time', 'amp'])
